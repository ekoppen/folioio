...
const Navigation = () => {
  ...
  useEffect(() => {
    const getAccentColor = () => {
      ...
      const accentValue = computedStyle.getPropertyValue('--dynamic-accent');

      if (accentValue && accentValue.trim() !== '') {
        // Convert HSL to a usable color format
        setAccentColor(`hsl(${accentValue})`);
      } else {
        // Fallback to blue
    ...
    // Set up observer for changes (in case accent color changes)
    const observer = new MutationObserver(getAccentColor);
  ...
  return (
    <>
    <header className={`fixed top-0 left-0 right-0 ${isMobileMenuOpen ? 'z-[10]' : 'z-[99998]'} transition-all duration-300 ${
      ...
      <nav className="container mx-auto px-4 sm:px-6 py-4 relative">
        <div className={`flex items-center h-6 ${siteSettings.logo_position === 'center' ? 'justify-center' : siteSettings.logo_position === 'right' ? 'justify-end' : 'justify-between'}`} style={{ height: '24px' }}>
            ...
            <div ref={logoRef} className="flex items-center gap-4">
              {siteSettings.logo_url && (
                <img 
                  ...
                  alt="Logo" 
                  className="w-auto absolute z-10 h-4 sm:h-5 md:h-6 lg:h-8"
                  style={{ 
              ...
              {siteSettings.show_site_title && (
                <button onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })} className="text-lg sm:text-xl md:text-2xl font-bold text-white">
              ...
              <div className="flex flex-col">
                {siteSettings.nav_title_visible && siteSettings.site_title && (
                  <div
                    className="font-semibold leading-tight text-sm sm:text-base md:text-lg"
                    style={{
                      ...
                      fontSize: `clamp(14px, ${Math.max(12, (siteSettings.nav_title_font_size || 24) * 0.6)}px, ${siteSettings.nav_title_font_size || 24}px)`,
                ...
                {siteSettings.nav_tagline_visible && siteSettings.site_tagline && (
                  <div
                    ...
                    style={{
                      ...
                      fontSize: `clamp(10px, ${Math.max(10, (siteSettings.nav_tagline_font_size || 14) * 0.7)}px, ${siteSettings.nav_tagline_font_size || 14}px)`,
          ...
          <div ref={navigationRef} className={`transition-opacity duration-200 ${isOverlapping ? 'opacity-0 pointer-events-none' : 'opacity-100'} hidden xl:flex items-center justify-center space-x-6 lg:space-x-8 absolute left-1/2 transform -translate-x-1/2 z-10`}>
            <button
          ...
          <div className="hidden xl:flex items-center space-x-4 absolute right-4 top-1/2 transform -translate-y-1/2 z-20">
            {/* <LanguageSwitcher /> */}
            {user ? (
              <>
                {isAdmin && (
          ...
          <div className="xl:hidden absolute right-4 top-1/2 transform -translate-y-1/2">
            <Button
              variant="ghost"
              size="sm"
    ...
    {/* Mobile Menu - Outside header to avoid CSS conflicts */}
    {isMobileMenuOpen && (
      <div
        style={{
          position: 'fixed',
          ...
          {pages.map((page) => (
            page.children && page.children.length > 0 ? (
              <div key={page.id} style={{ marginBottom: '16px' }}>
                <div style={{ color: 'rgba(255,255,255,0.7)', fontWeight: '500', marginBottom: '8px' }}>
                  {page.name}
                </div>
                {page.children.map((child) => (
                  <a
